...
**막힌 것 하나 — `ECOS_API_KEY`.** 키가 없어 `data/supply/rates.json` 이 아직
없다. 착공 패널은 정상이고 금리 패널만 "자료 없음" 으로 뜬다. `build_supply.py`
는 `rates.json` 이 없으면 종료코드 1 을 내므로, 키를 받기 전에는 크론
(`20 5 5,25 * *`)을 등록하지 않는다. 키를 `.env` 에 넣고
`./scripts/supply_monthly.sh` 를 한 번 돌린 뒤 등록한다.
