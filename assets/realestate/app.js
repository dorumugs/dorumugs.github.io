...
const METRIC_HELP = {
  level: '기준월까지 최근 3개월 동안 그 구에서 거래된 아파트의 평당가 중위값입니다'
    + '(현재 세대수 필터 기준). 한 달치만 쓰면 거래가 적은 구에서 어느 단지가 거래됐냐에 따라 '
    + '값이 10% 넘게 출렁여, 3개월을 모아 냅니다 — 대신 흐름이 바뀌는 시점은 한 달쯤 늦게 잡힙니다. '
  ...
  chg3: '현재 세대수 필터 기준으로, 기준월의 중위 평당가를 3개월 전 같은 값과 비교한 변화율입니다. '
    + '양쪽 다 최근 3개월을 모아 낸 중위값이라 거래 구성이 바뀌며 생기는 가짜 등락이 줄어듭니다. '
    + '최근 3개월치는 신고 지연으로 값이 계속 바뀔 수 있습니다.',
  ...
  chg12: '현재 세대수 필터 기준으로, 기준월의 중위 평당가를 12개월 전 같은 값과 비교한 변화율입니다. '
    + '양쪽 다 최근 3개월을 모아 낸 중위값이라 거래 구성이 바뀌며 생기는 가짜 등락이 줄어듭니다. '
    + '최근 3개월치는 신고 지연으로 값이 계속 바뀔 수 있습니다.',
  peak: '현재 세대수 필터 기준으로, 기준월까지 있었던 역대 최고 중위 평당가(3개월 기준) 대비 지금이 몇 % 인지입니다. '
...
function metricValue(code, metric, index) {
  ...
  // series.med 는 이미 최근 3개월 거래를 모아 낸 중위값이다(build_dashboard.py
  // MEDIAN_WINDOW). 여기서 또 이동중위를 씌우면 5개월치를 두 번 뭉개는 셈이라
  // 하지 않는다 — 얇은 달 보정도 창 안에서 이미 이뤄진다.
  const valueAt = (i) => {
