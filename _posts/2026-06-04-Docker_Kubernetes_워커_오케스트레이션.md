...
표만 보면 추상적이니까 가장 단순한 `Pod` 가 YAML 로 어떻게 생겼는지 한 번 보고 갑시다. Section 2 의 `docker run -d --name web -p 8080:80 nginx` 와 같은 일을 K8s 에게 시키면 이런 모양이에요.
...
`docker run` 은 "지금 이 컨테이너를 띄워" 라는 **명령** 이고, 위 YAML 은 "이런 상태로 있어 줘" 라는 **선언** 이에요. K8s 는 이 선언을 받아두고 실제로 그렇게 만들기 위해 노드를 골라 컨테이너를 띄웁니다. 죽으면 다시 띄우는 것도 같은 선언을 기준으로 동작해요.
...
처음 컨테이너 인프라를 익힐 때는 K8s 부터 들이대지 말고 **Docker 로 컨테이너 한두 개 띄워보는 감각** 을 먼저 잡는 게 훨씬 빠르게 와닿아요. 그 위에서 "여러 대에 걸쳐서 자동으로 굴러갔으면 좋겠다" 는 욕구가 생기는 시점이 K8s 를 펴들 타이밍입니다.
