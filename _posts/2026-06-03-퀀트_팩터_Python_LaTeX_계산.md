...
> - $\text{MACD}_t, \text{Signal}_t, \text{Hist}_t$ — 같은 시점 $t$ 의 세 값. **MACD ↔ Signal 의 차이가 Hist** 라는 구조만 잡으면 끝이에요.
...
- 무위험수익률($R_f$)을 빼는 이유 — "내가 위험을 감수해서 얻은 **추가** 수익" 만 평가하려는 거예요. 예금 이자보다 못한 펀드는 Sharpe 가 음수가 돼야 정상이에요.
