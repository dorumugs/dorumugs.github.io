---
layout: single
title:  "(5/5) LLM API 비용 추적 — Grafana 대시보드로 비용 한눈에 보기"
...
돌아보면 시작은 "이번 달 LLM 에 총 얼마 썼지?"라는 한 문장짜리 질문이었어요. 이제 그 질문은 아침 Slack 이 답하고, 초과는 알림이 잡고, 원인은 대시보드에서 10초 만에 찾고, 책임은 키 단위로 귀속됩니다. 비용 추적 시리즈는 여기서 완결이에요. 🎉

일단 오늘은 여기까지.....
다음에는 이 비용 데이터를 근거로 쉬운 작업을 싼 모델로 내리는 **모델 라우팅 자동화** 이야기를 해볼게요.

---

**← 이전 글:** [(4/5) LLM API 비용 추적 — 키가 난립할 때: 키 인벤토리와 키별 비용 귀속](/coding/LLM_API_비용추적_키별귀속/)
