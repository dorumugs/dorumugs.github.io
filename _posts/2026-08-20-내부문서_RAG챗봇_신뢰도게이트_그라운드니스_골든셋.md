---
layout: single
title:  "(3/6) 내부문서 RAG 챗봇 — 신뢰도 게이트, 그라운드니스와 골든셋"
...
2편에서 이미 넉넉히 뽑아뒀으니 버릴 여유도 있습니다.
떨어지는 비율은 코퍼스와 프롬프트에 따라 달라지는데, 이 글에서는 **10~20%가 떨어진다고 놓고** 이야기를 이어갈게요.
...
통과율 분포를 예시로 놓으면 이렇게 됩니다. 게이트가 **어디서 얼마나 떨어뜨리는지** 를 보는 용도예요.
...
돌리면 이런 표가 나옵니다. **아래 값은 예시**이고, 숫자는 문서·모델에 따라 달라지니 **모양만** 보세요.
...
| 임베딩이 미정 | **재서 정하는 절차**가 생김 (예시값에서는 로컬+리랭커 우세) |
