...
현실에서 쓰이는 건 거의 다 **Tools** 입니다. 그리고 n8n 의 MCP Server Trigger 가 노출하는 것도 정확히 도구예요. n8n 공식 문서는 이 노드를 두고 "일반 트리거 노드와 달리 **도구 노드에만 연결되고 도구 노드만 실행한다**" 고 못 박습니다. 클라이언트는 도구 목록을 받아가고, 개별 도구를 호출하죠.
