...
`grep` 은 진짜 단순해요. **파일 안에서 텍스트 패턴을 찾는 도구**. 1973년 유닉스에서 처음 나왔는데, 검색 도구 중에서는 거의 가장 오래된 축에 들어요.
...
정리하면 **의미적 검색(semantic search)** 이 필요한 모든 작업이에요.
...
> 💡 요점은 이거예요. **의미적 검색이 필요하면 LSP, 텍스트 검색이 필요하면 grep.** 둘은 경쟁이 아니라 서로 보완하는 관계예요.
