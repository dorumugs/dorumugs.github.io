...
categories: coding
header:
  image: /assets/images/2026-05-30-mssql-migration-troubleshooting/header.svg
  teaser: /assets/images/2026-05-30-mssql-migration-troubleshooting/header.svg
...
> 💡 이 글에서 다루는 것
> - **정합성 검증이 처음이라면** — 행 수 · CHECKSUM_AGG · DBCC CHECKDB 3종 도구와 한계
...
마이그레이션이 끝났는지 판단하려면 **"양쪽 데이터가 같다"를 어떻게 증명할지** 부터 알아야 해요. 아래 8가지 케이스가 전부 이 세 도구를 돌려가며 증상을 잡고 고친 뒤 다시 확인하니, 먼저 도구함부터 열고 갈게요.

| 도구 | 무엇을 보나 | 비용 | 못 잡는 것 |
|---|---|---|---|
| `COUNT(*)` | 행 개수가 같은가<br>(1차 스크리닝) | 매우 쌈 | 값이 틀려도<br>개수만 같으면 통과 |
| `CHECKSUM_AGG` | 테이블 전체 *값*이<br>같은가 | 보통 | collation/타입 차이,<br>드문 해시 충돌 |
| `DBCC CHECKDB` | 물리적 *무결성*<br>(페이지 손상) | 비쌈 | 논리적 값 불일치 |

쓰는 순서는 **싼 것 → 비싼 것**이에요. 행 수로 1차로 거르고, 값까지 보려면 CHECKSUM_AGG, 마지막에 물리 무결성은 DBCC CHECKDB.

### 행 수 비교 — 가장 빠른 1차

```sql
SELECT COUNT(*) FROM dbo.Orders;   -- 원본 / 타깃 각각 돌려 비교
```

개수가 다르면 볼 것도 없이 빠지거나 중복된 행이 있는 거예요. 단, **개수가 같다고 안심하면 안 돼요.** 값만 틀어진 UPDATE 누락은 개수로 안 잡혀요.

### 값 비교 — CHECKSUM_AGG

```sql
SELECT COUNT(*) AS cnt,
       CHECKSUM_AGG(BINARY_CHECKSUM(*)) AS chk
FROM dbo.Orders;
```

`BINARY_CHECKSUM(*)` 가 한 행의 모든 컬럼 값을 묶어 정수 지문 하나로 만들고, `CHECKSUM_AGG` 가 그걸 테이블 전체로 합산해요. 양쪽 `chk` 가 같으면 값까지 일치한다고 봐도 돼요.

> ⚠️ **CHECKSUM 의 한계도 알고 쓰세요.** ① collation·대소문자·trailing space 차이를 "같다"고 넘길 수 있고(아래 케이스 6과 연결), ② 아주 드물게 서로 다른 데이터가 같은 체크섬을 내는 충돌이 있어요. 100% 보증이 필요한 핵심 테이블은 `EXCEPT` 로 행 단위 차집합까지 봐주세요.
>
> ```sql
> -- 원본에는 있는데 타깃엔 없는 행 (양쪽을 같은 곳으로 끌어와 비교)
> SELECT * FROM source.dbo.Orders
> EXCEPT
> SELECT * FROM target.dbo.Orders;
> ```
...
페이지 손상·할당 오류 같은 *물리적* 문제를 봐요. 값이 맞는지(논리)는 위 두 도구가, DB 자체가 안 깨졌는지(물리)는 CHECKDB 가 담당이에요.

> 💡 한 줄 요약 — **행 수(빠름) → CHECKSUM_AGG(값) → DBCC CHECKDB(무결성)** 순으로 좁혀가면 돼요. 아래 케이스들은 이 셋을 반복해서 씁니다.
...
## 7. 케이스 5 — Computed column / rowversion
...
## 9. 케이스 7 — 트리거 fire-on-replication
