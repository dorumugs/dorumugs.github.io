...
toc: true
header:
  image: /assets/images/2026-06-04-k8s-yaml-7-objects/header.svg
  teaser: /assets/images/2026-06-04-k8s-yaml-7-objects/header.svg
---
...
| 자주 같이 쓰는 오브젝트 | 거의 모든 다른 오브젝트가 `metadata.namespace` 로 소속됨 |
...
이 한 줄짜리 오브젝트가 깔리면, 앞으로 만들 `Pod` / `Service` / `Deployment` 같은 오브젝트들이 모두 `web-demo` 라는 네임스페이스 안에 모이게 돼요. 다른 네임스페이스에서는 따로 적지 않으면 안 보입니다.
...
| 자주 같이 쓰는 오브젝트 | Pod 를 직접 만드는 일은 드물고, 대부분 `Deployment` 가 대신 만들어줌 |
...
| 자주 같이 쓰는 오브젝트 | 거의 항상 `Service` 와 짝. `Deployment` 가 만든 Pod 들 앞에 `Service` 가 안정 주소 |
...
| 자주 같이 쓰는 오브젝트 | `Deployment` (뒤에 깔리는 Pod 묶음), `Ingress` (Service 를 외부에 노출) |
...
> 💡 Service 의 종류(`type`) 가 몇 가지 있어요(`ClusterIP` 기본 / `NodePort` / `LoadBalancer`). 입문 단계에선 클러스터 내부용인 `ClusterIP` 만 알아도 충분해요. 외부 노출은 다음 항목인 `Ingress` 가 맡습니다.
...
| 자주 같이 쓰는 오브젝트 | `Service` (뒤에서 받는 쪽), Ingress Controller(NGINX, Traefik 등 실제로 트래픽을 처리하는 데몬) |
...
| 자주 같이 쓰는 오브젝트 | `Deployment` (env 또는 볼륨으로 주입), `Secret` (비밀값은 Secret 으로 짝) |
...
| 자주 같이 쓰는 오브젝트 | `Deployment` (env/볼륨으로 주입), 외부 시크릿 매니저(Vault, AWS Secrets Manager 등) |
...
이 7개만 익히면 K8s YAML 의 **80% 정도** 는 읽힙니다. 나머지는 이런 오브젝트들이에요.
...
이 오브젝트들도 결국 위에서 본 4개의 최상위 키(`apiVersion`/`kind`/`metadata`/`spec`) 골격을 그대로 따라요. 한 번 길이 보이면 나머지는 같은 패턴이에요.
