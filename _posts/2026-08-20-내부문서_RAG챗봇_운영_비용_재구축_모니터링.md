---
layout: single
title:  "(5/6) 내부문서 RAG 챗봇 — 재구축 가능한 캐시, GPU 비용, 모니터링"
...
1편에서 운영성을 2.0 → 1.5 로 **깎아서** 적은 게 이거였습니다. 정직하게 예상했고, 다섯 편을 지나며 그 예상이 그대로 확인됐어요.
...
위에 적은 "아직 약한 곳" 세 가지를 다시 읽어보니 걸리더라고요. **약점을 정직하게 나열한 것과 그 약점을 고친 것은 다른 일**이니까요.

특히 이 두 줄이요.

> `gate_version` 을 잊으면 옛 답이 계속 나갑니다. (4편 지뢰 ⑥)
> 재캘리브레이션을 잊지 마세요. (3편)

**지뢰 목록에 적어뒀다는 건 자주 일어난다는 걸 이미 알고 있었다는 뜻입니다.** 알면서 "잊지 마세요"로 둔 건 설계가 아니라 미룬 거예요.

[6편](/coding/내부문서_RAG챗봇_8점5에서_9점4로_2차채점/)에서 그 자리들을 구조로 바꿉니다 — 즉답 게이트를 서로 다른 두 자로 이중화하고, 근거가 죽으면 즉답이 **자동으로** 멈추게 하고, 사람이 정하는 임계값을 6개에서 1개로 줄여요. 2차 채점 결과는 **9.4점**이고, 왜 10점은 살 수 없는지도 거기서 다룹니다.
