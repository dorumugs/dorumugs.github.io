...
앞 글의 함수들은 단일 시리즈(`pd.Series`)를 받았어요. 멀티팩터 알파는 본질적으로 **여러 종목을 가로지르는 비교** 라서, 패널 데이터로 한 단계 확장합니다.
...
- **사후적으로 가중치를 튜닝**(예: 회귀로 IC 최대화) 하더라도 그 결과를 **out-of-sample** 에서 다시 검증해야 의미가 있어요. 이건 후속 글 주제.
...
$t$ 시점에 계산한 알파/시그널은 $t$ 시점의 종가까지를 보고 만든 값이라, **다음 거래일 수익률** 에 곱해야 정직한 백테스트가 됩니다.
...
- **수익률 분포가 왜곡되면 MDD 추정이 낙관적** 으로 나옵니다. 백테스트 기간에 큰 위기가 없었다면 MDD 는 작게 나오는 게 당연. 다른 기간으로 walk-forward 해보거나 부트스트랩으로 MDD 분포를 보는 게 정직해요.
...
- **회전율이 너무 낮으면** 시그널이 거의 안 바뀐다는 뜻 — 슬로우 팩터(가치/퀄리티) 면 자연스럽고, RSI 같은 단기 팩터로 회전율 5% 면 시그널이 안 살아있는 것일 가능성이 큽니다.
- **거래비용 가정의 함정** — 백테스트에서 Sharpe 2 였는데 15bp 비용 깔면 Sharpe 0.5 로 떨어지는 경우 흔해요. 처음부터 **비용 포함 Sharpe** 를 보는 게 정직합니다.
...
- **단일 기간 백테스트의 과적합** — 한 기간에서만 좋아 보이는 가중치/임계값은 walk-forward 또는 train/test 분리로 검증해야 의미가 있어요.
- **In-sample 가중치 튜닝** — 학습 구간에서 IC 최대 가중치를 찾고 같은 구간 Sharpe 를 자랑하는 건 자기 자신을 속이는 일. 늘 out-of-sample 평가.
