---
layout: single
title:  "(2/3) 조건부 쓰기(If-Match/ETag)로 경쟁 구간까지 닫기 — S3 원자적 버전 가드"
...
일단 오늘은 여기까지.....   
다음 글에서는 지금까지 "신뢰된 편집자끼리의 실수" 를 막았다면, 한 걸음 더 나가서 **애초에 권한 없는 사람이 저장하지 못하게** 쓰기 경로에 접근 제한을 거는 이야기를 정리해볼게요.

---

**← 이전 글:** [(1/3) 백엔드 없이 브라우저→S3 공유 편집, 덮어쓰기 유실 막기 (rev 버전 가드)](/coding/브라우저_S3_직접저장_동시편집_버전가드/) ｜ **다음 글 →:** [(3/3) 누가 저장할 수 있는지 — 백엔드 없이 쓰기 경로에 접근 제한 걸기](/coding/S3_쓰기경로_접근제한_누가_저장/)
