---
layout: single
title:  "(6/6) 2D SRPG 만들기 선행학습 — 데이터 설계와 세이브/로드"
...
description: "2D SRPG 선행학습 마지막 6편. 하드코딩된 능력치를 JSON 데이터로 빼내는 데이터 주도 설계, 스킬을 데이터로 정의하는 법, 세이브/로드의 원칙(상태만 저장·버전 필드)을 다루고, 시리즈 이후의 학습 로드맵으로 마무리합니다."
---
...
마지막 편은 이 문제를 푸는 **데이터 주도 설계(data-driven design)** 와, 게임을 껐다 켜도 이어할 수 있게 하는 **세이브/로드**를 다룹니다. 그리고 시리즈를 마치며, 여기서 더 나아가려면 무엇을 배우면 되는지 로드맵을 남겨둘게요.
...
1. **맵 에디터** — 배열 손편집은 금방 한계가 와요. [Tiled](https://www.mapeditor.org) 라는 무료 에디터로 맵을 그리고, `pytmx` 라이브러리로 읽어오는 조합이 표준입니다. 2편의 배열 자리에 그대로 꽂혀요.
...
여섯 편을 함께 달려오시느라 고생 많으셨어요. 이 시리즈의 코드 조각들은 전부 이어 붙이면 돌아가도록 짰으니, 꼭 직접 실행하고 숫자를 바꿔보면서 자기 것으로 만드시길 바랍니다.
...
**← 이전 글:** [(5/6) 2D SRPG 만들기 선행학습 — 턴제 전투와 상태머신](/coding/SRPG_만들기_선행학습_턴제_전투/)
