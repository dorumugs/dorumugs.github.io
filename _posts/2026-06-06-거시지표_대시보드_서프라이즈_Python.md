...
toc: true
header:
  image: /assets/images/2026-06-06-macro-dashboard-surprise/header.svg
  teaser: /assets/images/2026-06-06-macro-dashboard-surprise/header.svg
...
codes = {"CPIAUCSL": "CPI", "UNRATE": "실업률", "DGS10": "국채10년",
         "VIXCLS": "VIX", "BAMLH0A0HYM2": "신용스프레드"}
...
이러면 절대값이 아니라 **상대 변화** 로 비교돼서, "금리는 두 배가 됐는데 CPI 는 완만하게 올랐고, 신용 스프레드는 평소엔 잠잠하다 충격 때만 확 튀네" 같은 비교가 한 화면에서 됩니다. 특히 신용 스프레드는 단위(%p)가 작아서 절대값으로 그리면 큰 숫자에 묻히는데, 정규화하면 "출발선 대비 몇 배로 벌어졌나" 로 환산돼서 위험 신호가 또렷하게 살아나요.
